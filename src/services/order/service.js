...
class AuthService {
  ...
  static async googleCallback(req, res) {
    ...
    try {
      ...
      let [user] = (await UserResource.findAll({ email })).data ?? [];

      if (!user) {
        // Register new user and return registered data
        user = (await UserResource.insert({email, name, picture, type: AUTH_TYPE.GOOGLE}));
      }
